...
- `replaceOrgLatex`: If `true` (default), converts Org-mode [[MY SITE/docs/features/Latex|Latex]] fragments to Quartz-compatible LaTeX wrapped in `$` (for inline) and `$$` (for block equations).
